...
PUBLIC_KEY = None
NAME = None
PERSISTENT_ID = None  # For stable user identity
...
def generate_persistent_id():
    """Generate a persistent user ID based on public key"""
    global PUBLIC_KEY
    if PUBLIC_KEY:
        ...
        # Create base64-like ID
        persistent_id = base64.b64encode(hash_obj.digest())[:43].decode('ascii').replace('+', '-').replace('/', '_')
...
def change_password():
    ...
    with _data_lock:
        ...
        try:
            ...
            save_encrypted_data(USER_PASSWORD, {"name": NAME, "persistent_id": PERSISTENT_ID}, "/usr/share/davell/storage/PersonalInfo.enc", use_password=True)
...
def load_user_data():
    global friends, pending_requests, PERSISTENT_ID, NAME
    ...
    with _data_lock:
        try:
            ...
            if isinstance(personal_data, dict):
                ...
                PERSISTENT_ID = personal_data.get("persistent_id", PERSISTENT_ID)
                
                # If no persistent ID exists, generate one
...
def initialize_system(pid):
    global USER_PASSWORD, PERSISTENT_CERT, PUBLIC_KEY, NAME, PERSISTENT_ID
    ...
    if os.path.exists("/usr/share/davell/storage/certs/persistent_cert.pem"):
        ...
        try:
            ...
            if isinstance(personal_data, dict):
                ...
                PERSISTENT_ID = personal_data.get("persistent_id")
            ...
            NAME = f"EMSuser_{random.randint(100000, 999999)}"
            PERSISTENT_ID = generate_persistent_id()
            save_encrypted_data(USER_PASSWORD, {"name": NAME, "persistent_id": PERSISTENT_ID}, "/usr/share/davell/storage/PersonalInfo.enc", use_password=True)
        ...
        Message.info(f"Your persistent ID: {PERSISTENT_ID}")
...
def create_new_system():
    global USER_PASSWORD, PERSISTENT_CERT, PUBLIC_KEY, NAME, friends, pending_requests, PERSISTENT_ID
...
Log notice file /usr/share/davell/tor/tor.log
...
TOR_SOCKS_PORT = 9050  # Default, may be updated in setup
...
class GUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Davell Messenger")
    ...
    def load_real_data(self):
        ...
        global friends, pending_requests, NAME, PERSISTENT_ID
    ...
    def change_name(self):
        ...
        if new_name and 3 <= len(new_name) <= 50:
            ...
            try:
                save_encrypted_data(USER_PASSWORD, {"name": NAME, "persistent_id": PERSISTENT_ID}, 
    ...
    def show_security_status(self):
        ...
        status_text = (
            ...
            f"Persistent ID: {'✓ Set' if PERSISTENT_ID else '✗ Missing'}\n"
            f"Friends: {len(friends)}\n"
            f"Pending requests: {len(pending_requests)}\n"
            f"Server port: {PORT}\n"
...
def app(port=PORT):
    try:
        ...
        if os.path.exists("/usr/share/davell/storage/certs/persistent_cert.pem"):
            ...
            Message.error("Missing Certs. persistent_cert.pem")
